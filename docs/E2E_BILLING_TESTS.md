...
## Cenario B - Upgrade Pro com cartão
...
## Cenario C - Confirmação fallback pos-checkout

Objetivo: validar robustez quando retorno do checkout não volta automaticamente ao app.

1. Finalizar pagamento no MP sem voltar para o site.
2. No app, acionar confirmação manual (endpoint):
...
1. Usuário com plano pago abre `/settings`.
...
- Admin/moderator não passam por cobranca.
- Webhook e confirmação fallback mantem consistencia do plano.
