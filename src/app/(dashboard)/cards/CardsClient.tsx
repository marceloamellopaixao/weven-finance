...
export function CardsClient() {
  ...
  const handleSave = async () => {
    if (!activeCard || activeCard.type === "debit_card") {
      setFeedback({ type: "info", message: tCards("feedback.debitNoInvoiceLimit") });
      return;
    }
    setIsSaving(true);
    setFeedback(null);
    try {
      ...
      setFeedback({ type: "error", message: error instanceof Error ? error.message : tCards("feedback.saveSettingsError") });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSyncHistorical = async () => {
    ...
    try {
      ...
      setFeedback({
        type: "info",
        message: count > 0 ? tCards("feedback.syncDone", { count }) : tCards("feedback.syncNone"),
      });
    } catch (error) {
      setFeedback({ type: "error", message: error instanceof Error ? error.message : tCards("feedback.syncError") });
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleSaveCard = async () => {
    ...
    if (!cardBankName.trim() || safeLast4.length !== 4) {
      setFeedback({ type: "error", message: tCards("feedback.missingCardFields") });
      return;
    }
    ...
    if (isCreditFunction && cardDueDate) {
      ...
      if (day < 1 || day > 31) {
        setFeedback({ type: "error", message: tCards("feedback.invalidDueDay") });
        return;
      }
    }
    if (isCreditFunction && cardClosingDay) {
      ...
      if (day < 1 || day > 31) {
        setFeedback({ type: "error", message: tCards("feedback.invalidClosingDay") });
        return;
      }
    }
    ...
    try {
      ...
      setFeedback({ type: "error", message: error instanceof Error ? error.message : tCards("feedback.cardSaveError") });
    } finally {
  ...
  const handleDeleteCard = async (cardId: string) => {
    ...
    try {
      ...
      setFeedback({ type: "success", message: tCards("feedback.cardRemoved") });
    } catch (error) {
      setFeedback({ type: "error", message: error instanceof Error ? error.message : tCards("feedback.cardRemoveError") });
    } finally {
      setIsSavingCard(false);
    }
  };
  ...
  const renderCardFace = (input: { bankName: string; last4: string; type: PaymentCardType; brand?: string; dueDate?: number; className?: string }) => {
    ...
    const displayName = userProfile?.displayName || tCards("cardFace.clientFallback");
    ...
    return (
      <div className={`relative overflow-hidden rounded-2xl shadow-2xl transition-all duration-300 ${theme.background} ${theme.shadow} ${input.className || ""}`}>
        ...
        <div className={`relative aspect-[1.58/1] min-h-[220px] w-full p-5 md:p-6 flex flex-col justify-between ${theme.foreground}`}>
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-[0.18em] opacity-80">{tCards("cardFace.bank")}</p>
              <p className="text-xl font-bold leading-tight tracking-wide">{input.bankName || tCards("cardFace.bankFallback")}</p>
            </div>
            <Badge variant="outline" className={`text-[10px] uppercase tracking-widest font-semibold ${theme.badge}`}>
              {input.type === "credit_card" ? tCards("cardType.creditCardFace") : input.type === "debit_card" ? tCards("cardType.debitCardFace") : tCards("cardType.multipleCardFace")}
            </Badge>
          </div>
          ...
          <div className="flex items-end justify-between mt-auto">
            <div>
              <p className={`text-[10px] uppercase tracking-widest ${theme.muted}`}>{tCards("cardFace.cardholder")}</p>
              <p className="text-sm font-semibold tracking-wider">{displayName.slice(0, 22).toUpperCase()}</p>
            </div>
            <div className="text-right">
              {isCreditFunction && input.dueDate ? (
                <>
                  <p className={`text-[10px] uppercase tracking-widest ${theme.muted}`}>{tCards("cardFace.dueDate")}</p>
                  <p className="text-[13px] font-semibold tracking-wider">{tCards("cardFace.dueDay", { day: input.dueDate })}</p>
                </>
              ) : (
                <p className={`text-[11px] font-semibold ${theme.muted}`}>{tCards("cardFace.defaultDue")}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto space-y-6 md:space-y-8">
        ...
        <div id="tour-cards-header" className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="grid w-full grid-cols-1 gap-2 sm:flex sm:w-auto">
            <Button onClick={handleSyncHistorical} variant="outline" size="icon" className="h-10 w-full rounded-xl border-border/70 bg-card sm:h-10 sm:w-10 sm:rounded-full" disabled={isSyncing || showCardForm} title={tCards("actions.syncHistory")} aria-label={tCards("actions.syncHistory")}>
        ...
        {!showCardForm && paymentCards.length === 0 && (
          <Card className="rounded-3xl border-dashed bg-transparent shadow-none">
            <CardContent className="flex min-h-[300px] md:min-h-[360px] flex-col items-center justify-center py-8 md:py-12 text-center">
              ...
              <h3 className="text-lg font-semibold">{tCards("empty.title")}</h3>
              <p className="mt-2 mb-6 max-w-sm px-2 text-sm text-muted-foreground">
                {tCards("empty.description")}
              </p>
              <Button
          ...
          <Card className="rounded-3xl border border-border/70 bg-card shadow-xl animate-in fade-in slide-in-from-bottom-4">
            <CardHeader className="border-b border-border/70 pb-4">
              <div className="flex justify-between items-center">
                <CardTitle>{editingCardId ? tCards("form.editTitle") : tCards("form.newTitle")}</CardTitle>
                <Button
                  type="button"
                  variant="ghost"
                  size="icon"
                  aria-label={tCards("form.closeAria")}
              ...
              <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)] xl:items-start">
                <div className="space-y-4">
                  ...
                  <div className="app-panel-soft rounded-3xl border p-4">
                    <p className="text-sm font-semibold text-foreground">{tCards("form.binTitle")}</p>
                    <p className="mt-1 text-sm text-muted-foreground">
                      {editingCardId
                        ? tCards("form.binEditDescription")
                        : tCards("form.binNewDescription")}
                    </p>

                    <div className="mt-4 rounded-2xl border border-primary/15 bg-background/80 p-4">
                      <div className="space-y-2">
                        <Label className="flex items-center gap-1">
                          {tCards("form.binLabel")}
                          <span className="text-xs font-normal text-muted-foreground">{tCards("form.optional")}</span>
                        </Label>
                        <Input value={cardBin} maxLength={8} inputMode="numeric" placeholder={tCards("form.binPlaceholder")} onChange={(e) => setCardBin(e.target.value.replace(/\D/g, "").slice(0, 8))} />
                        <div className="min-h-4">
                          {isIdentifyingCard ? (
                            <p className="flex items-center gap-1 text-xs text-primary animate-pulse">
                              <RefreshCw className="h-3 w-3 animate-spin" /> {tCards("form.identifyingBank")}
                            </p>
                          ) : cardBrand ? (
                            <p className="flex items-center gap-1 text-xs text-emerald-600">
                              <CheckCircle2 className="h-3 w-3" /> {cardBankName || tCards("form.detectedBankFallback")} • {cardBrand}
                            </p>
                          ) : (
                            <p className="text-xs text-muted-foreground">{tCards("form.binHelp")}</p>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-5">
                    <div className="space-y-2 sm:col-span-2">
                      <Label>{tCards("form.bankName")}</Label>
                      <Input value={cardBankName} maxLength={40} list="bank-options" placeholder={tCards("form.bankPlaceholder")} onChange={(e) => setCardBankName(e.target.value)} />
                    </div>

                    <div className="space-y-2">
                      <Label>{tCards("form.last4")}</Label>
                      <Input value={cardLast4} maxLength={4} inputMode="numeric" placeholder={tCards("form.last4Placeholder")} onChange={(e) => setCardLast4(e.target.value.replace(/\D/g, "").slice(0, 4))} />
                    </div>

                    <div className="space-y-2">
                      <Label>{tCards("form.cardFunction")}</Label>
                      <Select value={cardType} onValueChange={(v) => setCardType(v as PaymentCardType)}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="credit_card">{tCards("cardType.credit")}</SelectItem>
                          <SelectItem value="debit_card">{tCards("cardType.debit")}</SelectItem>
                          <SelectItem value="credit_and_debit">{tCards("cardType.multipleFull")}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    {(cardType === "credit_card" || cardType === "credit_and_debit") && (
                      <div className="grid gap-4 sm:col-span-2 sm:grid-cols-2">
                        <div className="space-y-2">
                          <Label>{tCards("form.closingDay")}</Label>
                          <Input value={cardClosingDay} inputMode="numeric" placeholder={tCards("form.closingDayPlaceholder")} className="w-full" onChange={(e) => setCardClosingDay(e.target.value.replace(/\D/g, "").slice(0, 2))} />
                        </div>
                        <div className="space-y-2">
                        <Label>{tCards("form.dueDay")}</Label>
                          <Input value={cardDueDate} inputMode="numeric" placeholder={tCards("form.dueDayPlaceholder")} className="w-full" onChange={(e) => setCardDueDate(e.target.value.replace(/\D/g, "").slice(0, 2))} />
                        </div>
                      </div>
                    )}
                  </div>
              ...
              >
                {paymentCards.length > 1 && (
                  <>
                    <button
                      type="button"
                      aria-label={tCards("carousel.previous")}
                    ...
                    <button
                      type="button"
                      aria-label={tCards("carousel.next")}
                ...
                <div className="relative min-h-[232px] sm:min-h-[252px]">
                  ...
                  <button
                    type="button"
                    aria-label={tCards("carousel.editAria", { name: activeCard.bankName || tCards("carousel.selectedFallback") })}
                ...
                <div className="mt-5 grid grid-cols-1 gap-3 sm:flex sm:flex-wrap sm:items-center sm:justify-center sm:gap-3">
                  <Button variant="outline" size="sm" className="rounded-full border-border/70 bg-card shadow-sm hover:bg-accent" onClick={() => handleEditCard(activeCard)}>
                    <Pencil className="mr-2 h-3.5 w-3.5" /> {tCards("actions.edit")}
                  </Button>
                  <Button variant="outline" size="sm" className="rounded-full border-red-200 bg-card shadow-sm text-red-600 hover:bg-red-50 hover:text-red-700" onClick={() => handleDeleteCard(activeCard.id)}>
                    <Trash2 className="mr-2 h-3.5 w-3.5" /> {tCards("actions.delete")}
                  </Button>
                  <Button id="tour-cards-add-button" type="button" variant="default" size="sm" aria-label={tCards("carousel.addNewAria")} className="rounded-full shadow-sm" onClick={() => setShowCardForm(true)}>
                    <Plus className="mr-2 h-4 w-4" /> {tCards("actions.newCard")}
                  </Button>
                </div>
                  ...
                  <div className="mt-5 flex items-center justify-center gap-1">
                    {paymentCards.map((card, index) => (
                      <button
                        key={card.id}
                        type="button"
                        aria-label={tCards("carousel.selectCard", { index: index + 1 })}
              ...
              <Card className="rounded-3xl border border-border/70 bg-card shadow-sm">
                <CardContent className="space-y-4 p-5">
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{tCards("selected.title")}</p>
                      <p className="mt-1 truncate text-base font-semibold text-foreground">{tCards("selected.cardEnding", { bank: activeCard.bankName, last4: activeCard.last4 })}</p>
                    </div>
                    <Badge variant="secondary" className="app-panel-subtle shrink-0 text-muted-foreground">
                      {activeCard.type === "credit_card" ? tCards("cardType.credit") : activeCard.type === "debit_card" ? tCards("cardType.debit") : tCards("cardType.multiple")}
                    </Badge>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                      <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tCards("selected.bin")}</p>
                      <p className="mt-2 font-semibold text-foreground">{activeCard.bin || tCards("selected.notProvided")}</p>
                    </div>
                    <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                      <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{tCards("selected.brand")}</p>
                      <p className="mt-2 font-semibold text-foreground">{activeCard.brand || tCards("selected.manual")}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="space-y-6">
                ...
                <CardContent className="space-y-6 p-6 md:p-8">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <h2 className="flex items-center gap-2 text-lg font-bold text-foreground">
                        <ShieldAlert className="h-5 w-5 text-primary" /> {tCards("limits.title")}
                      </h2>
                      <p className="mt-1 text-sm text-muted-foreground">
                        {tCards("limits.description")}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant="secondary" className="app-panel-subtle font-medium text-muted-foreground">{tCards("limits.perCard")}</Badge>
                      {activeCard.type !== "debit_card" && (
                        <Button variant={showSettings ? "default" : "outline"} size="sm" className="rounded-full" onClick={() => setShowSettings((prev) => !prev)}>
                          ...
                          {showSettings ? tCards("actions.hideSettings") : tCards("actions.adjustRules")}
                        </Button>
                      )}
                    </div>
                  </div>
                  ...
                  {activeCard.type === "debit_card" ? (
                    <div className="rounded-2xl border border-primary/20 bg-accent px-4 py-3 text-sm text-primary">
                      {tCards("limits.debitNotice")}
                    </div>
                  ) : (
                    <>
                      <div className="grid gap-4 md:grid-cols-3">
                        <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                          <p className="text-sm font-medium text-muted-foreground">{tCards("limits.cardLimit")}</p>
                          <p className="financial-value mt-2 text-xl font-bold text-foreground sm:text-2xl">{money(activeCardCreditSummary?.limit || 0)}</p>
                        </div>
                        <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                          <p className="text-sm font-medium text-muted-foreground">{tCards("limits.invoiceUsed")}</p>
                          <p className="financial-value mt-2 text-xl font-bold text-foreground sm:text-2xl">{money(activeCardCreditSummary?.used || 0)}</p>
                        </div>
                        <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
                          <p className="text-sm font-medium text-muted-foreground">{tCards("limits.available")}</p>
                          <p className={`financial-value mt-2 text-xl font-bold sm:text-2xl ${(activeCardCreditSummary?.available || 0) < 0 ? "text-red-600" : "text-emerald-600"}`}>
                            {money(activeCardCreditSummary?.available || 0)}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-col gap-2 text-sm sm:flex-row sm:items-center sm:justify-between">
                          <span className="font-medium text-muted-foreground">{tCards("limits.usedPercent", { value: formatNumber(activeCardCreditSummary?.usagePct || 0, { minimumFractionDigits: 1, maximumFractionDigits: 1 }) })}</span>
                          {danger && <span className="flex items-center gap-1 font-semibold text-red-600"><AlertTriangle className="h-4 w-4" /> {tCards("limits.exceeded")}</span>}
                          {warning && !danger && <span className="font-semibold text-amber-600">{tCards("limits.warning")}</span>}
                        </div>
                        <div className="pt-1">
                          <Link href={`/piggy-bank/new?goal=card_limit&cardId=${encodeURIComponent(activeCard.id)}`}>
                            <Button className="rounded-xl bg-primary text-primary-foreground hover:bg-primary/90">
                              <PiggyBank className="mr-2 h-4 w-4" />
                              {tCards("actions.increaseLimitGoal")}
                            </Button>
                          </Link>
                        </div>
                      </div>
                  ...
                  <div className="border-t border-border/70 pt-6">
                    <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-muted-foreground">{tCards("limits.rulesTitle")}</h3>
                        <p className="mt-1 text-sm text-muted-foreground">
                          {activeCard.type === "debit_card"
                            ? tCards("limits.debitRulesDescription")
                            : tCards("limits.creditRulesDescription")}
                        </p>
                      </div>
                      {activeCard.type !== "debit_card" && !showSettings && (
                        <div className="flex flex-wrap gap-2">
                          <Badge variant="outline" className="rounded-full border-border/70 bg-background/80 px-3 py-1 text-xs text-muted-foreground">{tCards("limits.alertBadge", { value: settings.alertThresholdPct })}</Badge>
                          <Badge variant="outline" className="rounded-full border-border/70 bg-background/80 px-3 py-1 text-xs text-muted-foreground">{settings.enabled ? tCards("limits.controlActive") : tCards("limits.controlPaused")}</Badge>
                        </div>
                      )}
                    </div>
                    ...
                    {activeCard.type === "debit_card" ? (
                      ...
                      <div className="space-y-5 rounded-3xl border border-border/70 bg-background/70 p-5">
                        ...
                        <div className="app-panel-subtle flex flex-col gap-4 rounded-2xl border p-4 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            <p className="text-sm font-medium text-foreground">{tCards("limits.enableLimitControl")}</p>
                            <p className="text-xs text-muted-foreground">{tCards("limits.enableLimitControlDescription")}</p>
                          </div>
              ...
              <div className="space-y-4">
                <h3 className="flex items-center gap-2 px-2 text-lg font-bold text-foreground">
                  <ReceiptText className="h-5 w-5 text-primary" /> {tCards("recent.title")}
                </h3>
                <div className="rounded-3xl border border-border/70 bg-card p-2 shadow-sm">
                  {cardTransactions.length === 0 ? (
                    <div className="p-8 text-center">
                      <p className="text-muted-foreground">{tCards("recent.empty", { bank: activeCard.bankName, last4: activeCard.last4 })}</p>
                    </div>
                  ) : (
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
                                ...
                                <span className="shrink-0">{tCards("recent.dueDate", { date: tx.dueDate ? formatDate(tx.dueDate, { day: "2-digit", month: "2-digit", year: "numeric" }) : "" })}</span>
                              </div>
                            </div>
                          </div>
                          <div className="text-left sm:text-right">
                            <p className="font-bold text-foreground">{money(Number(tx.amount || 0))}</p>
                            <p className={`mt-1 text-[10px] font-semibold uppercase tracking-wider ${tx.status === "pending" ? "text-amber-600" : "text-emerald-600"}`}>
                              {tx.status === "pending" ? tCards("recent.pending") : tCards("recent.paid")}
                            </p>
                          </div>
                        </div>
                      ))}
