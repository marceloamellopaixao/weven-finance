...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    // Carregar preferência local de privacidade
    const localPrivacy = localStorage.getItem("weven_privacy_mode");
    if (localPrivacy === "true") setIsPrivacyMode(true);
  }, [userProfile]);
  ...
  const handlePrivacyToggle = (checked: boolean) => {
    ...
    localStorage.setItem("weven_privacy_mode", String(checked));
    // Aqui você poderia despachar um evento ou usar um Contexto para atualizar o Dashboard em tempo real
    // Por enquanto, funciona como preferência persistente.
  };
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      await logout(); // Redireciona para home
    } catch (error) {
      console.error(error);
      setIsDeleting(false);
    }
        ...
        <div className="space-y-6">
            ...
            <Card className="border-none shadow-xl shadow-zinc-200/50 dark:shadow-black/20 bg-white dark:bg-zinc-900 rounded-3xl animate-in fade-in zoom-in-95 duration-300">
              ...
              <CardContent className="space-y-8">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                  <div className="relative">
                    <Avatar className="h-24 w-24 border-4 border-zinc-50 dark:border-zinc-800 shadow-xl">
                      ...
                      <AvatarFallback className="text-2xl bg-zinc-100 dark:bg-zinc-800 text-zinc-500">
                  ...
                  <div className="space-y-1 text-center sm:text-left">
                    ...
                    <div className="flex flex-wrap justify-center sm:justify-start gap-2 pt-2">
                      <Badge variant="secondary" className={`uppercase text-[10px] tracking-wider border ${
                        currentPlan === 'free' 
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label className="text-zinc-500">Nome de Exibição</Label>
                    <Input 
                      value={name} 
                      onChange={(e) => setName(e.target.value)} 
                      className="h-11 rounded-xl border-zinc-200 dark:border-zinc-800 focus:ring-violet-500" 
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      disabled 
                      className="h-11 rounded-xl bg-zinc-50 dark:bg-zinc-950/50 border-zinc-200 dark:border-zinc-800 opacity-70 cursor-not-allowed" 
                    />
                    <p className="text-[10px] text-zinc-400">O e-mail não pode ser alterado por segurança.</p>
                  </div>
                </div>
              </CardContent>
              <CardFooter className="flex justify-end border-t border-zinc-50 dark:border-zinc-800/50 pt-6 bg-zinc-50/50 dark:bg-zinc-900/50 rounded-b-3xl">
                <Button 
                  ...
                  disabled={isSaving}
                  className="bg-violet-600 hover:bg-violet-700 text-white rounded-xl px-8 h-11 shadow-lg shadow-violet-500/20 transition-all active:scale-95"
                >
          ...
          {activeTab === "billing" && (
            <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
              ...
              {/* Card do Plano Atual */}
              <Card className={`border-none shadow-xl rounded-3xl relative overflow-hidden text-white ${
                currentPlan === 'free' 
                  ? 'bg-zinc-800 dark:bg-zinc-900' 
                  : 'bg-linear-to-br from-violet-600 to-indigo-700 shadow-violet-500/20'
              }`}>
                  ...
                  <div className="flex justify-between items-start z-10">
                    <div>
                      <CardTitle className="text-3xl flex items-center gap-2 font-bold">
                        {currentPlan === 'free' ? <Wallet className="h-8 w-8 text-zinc-400" /> : <Crown className="h-8 w-8 text-yellow-300 fill-yellow-300" />}
                        Weven {currentPlan.charAt(0).toUpperCase() + currentPlan.slice(1)}
                      </CardTitle>
                      <CardDescription className={`mt-2 text-lg ${currentPlan === 'free' ? 'text-zinc-400' : 'text-violet-100'}`}>
                        {currentPlan === 'free' 
                          ? "Você está utilizando a versão básica." 
                          : "Obrigado por apoiar nosso desenvolvimento!"}
                      </CardDescription>
                    </div>
                    <Badge className="bg-white/20 backdrop-blur-md hover:bg-white/30 text-white border-none flex gap-1.5 items-center px-3 py-1.5 text-sm">
                  ...
                  {currentPlan === 'free' && (
                    <div className="mt-4">
                      <p className="text-sm text-zinc-300 mb-6">
                        Faça o upgrade para remover limites e desbloquear todo o potencial.
                      </p>
                    </div>
                  )}
                </CardContent>
              ...
              {/* Opções de Upgrade (Só mostra se não for Premium) */}
              {currentPlan !== 'premium' && (
                <div className="grid gap-6 md:grid-cols-2">
                   {/* Card Pro */}
                   {currentPlan !== 'pro' && (
                     <Card className="border-2 border-violet-100 dark:border-violet-900/30 shadow-lg hover:shadow-xl hover:border-violet-300 transition-all bg-white dark:bg-zinc-900 rounded-3xl group cursor-pointer relative overflow-hidden">
                       ...
                       <CardHeader>
                         <CardTitle className="flex justify-between items-center">
                            <span className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-violet-500" /> Weven Pro</span>
                            <span className="text-xl font-bold text-zinc-900 dark:text-white">R$ 19,90</span>
                         </CardTitle>
                         <CardDescription>Transações ilimitadas + Controle de Streaming.</CardDescription>
                       </CardHeader>
                       <CardFooter>
                   ...
                   <Card className="border-2 border-emerald-100 dark:border-emerald-900/30 shadow-lg hover:shadow-xl hover:border-emerald-300 transition-all bg-white dark:bg-zinc-900 rounded-3xl group cursor-pointer relative overflow-hidden">
                     ...
                     <CardHeader>
                       <CardTitle className="flex justify-between items-center">
                          <span className="flex items-center gap-2"><Star className="h-5 w-5 text-emerald-500" /> Weven Premium</span>
                          <span className="text-xl font-bold text-zinc-900 dark:text-white">R$ 49,90</span>
                       </CardTitle>
                       <CardDescription>Tudo do Pro + Criptografia Exclusiva + Suporte VIP.</CardDescription>
                     </CardHeader>
                     <CardFooter>
                        <Link href={MERCADO_PAGO_LINKS.premium} target="_blank" className="w-full">
                          <Button variant="outline" className="w-full rounded-xl border-emerald-500 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 group-hover:scale-[1.02] transition-transform">
          ...
          {activeTab === "security" && (
            <Card className="border-none shadow-xl shadow-zinc-200/50 dark:shadow-black/20 bg-white dark:bg-zinc-900 rounded-3xl animate-in fade-in zoom-in-95 duration-300">
              <CardHeader>
                ...
                <CardDescription>
                  Controle total sobre seus dados e visibilidade.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-8">
                ...
                <div className="flex items-center justify-between p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-100 dark:border-zinc-800 transition-all hover:border-zinc-300 dark:hover:border-zinc-700">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Label className="text-base font-semibold text-zinc-900 dark:text-zinc-100">Modo Discreto (Blur)</Label>
                    </div>
                    <p className="text-sm text-zinc-500">Oculta valores monetários no Dashboard para privacidade em locais públicos.</p>
                  </div>
                  <Switch checked={isPrivacyMode} onCheckedChange={handlePrivacyToggle} className="data-[state=checked]:bg-violet-600" />
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-semibold text-sm uppercase tracking-wider text-zinc-500">Segurança de Dados</h3>
                  </div>
                  <div className="p-5 rounded-2xl bg-zinc-950 text-zinc-400 font-mono text-xs break-all relative border border-zinc-800 shadow-inner">
                    <div className="absolute top-3 right-3">
                      <Badge variant="outline" className="text-[10px] border-emerald-900 bg-emerald-950 text-emerald-500 font-bold px-2 py-0.5">E2EE ATIVO</Badge>
                    </div>
                    <p className="mb-2 text-zinc-600 uppercase tracking-widest text-[10px] font-bold">Identificador Seguro (Hash)</p>
                    {user?.uid ? `uid_hash_${user.uid.substring(0,8)}...e2ee_enabled` : "Carregando..."}
                  </div>
                  <p className="text-xs text-zinc-500 leading-relaxed">
                    * Seus dados sensíveis são criptografados antes de sair do seu dispositivo. Nem mesmo os desenvolvedores da Weven Finance possuem acesso aos valores brutos das suas transações.
                  </p>
                </div>
                ...
                <div className="pt-2">
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border border-red-100 dark:border-red-900/30 bg-red-50/50 dark:bg-red-950/10 rounded-2xl">
                    <p className="text-xs text-red-600/80 dark:text-red-400">
                      A exclusão da conta é <strong>irreversível</strong>. Todos os dados, histórico e configurações serão apagados permanentemente dos nossos servidores.
                    </p>
                    <Button 
                      variant="outline" 
                      onClick={() => setShowDeleteModal(true)}
                      className="text-red-600 border-red-200 hover:bg-red-100 hover:border-red-300 dark:hover:bg-red-900/40 dark:border-red-900 whitespace-nowrap rounded-xl"
                    >
                      Excluir Minha Conta
                    </Button>
                  </div>
        ...
        <Dialog open={showDeleteModal} onOpenChange={setShowDeleteModal}>
          <DialogContent className="sm:max-w-[425px] rounded-3xl p-6">
            ...
            <DialogFooter className="gap-2 sm:gap-0 mt-4">
              <Button variant="outline" onClick={() => setShowDeleteModal(false)} className="rounded-xl h-10 w-full sm:w-auto">Cancelar</Button>
              <Button 
                variant="destructive" 
