...
export type TransactionsArgs = WorkspaceScope & { month?: string; type?: string; page?: number; pageSize?: number; syncRecurring?: boolean };

export const transactionsApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    getTransactions: build.query<Transaction[], TransactionsArgs>({
      queryFn: async ({ userId, workspaceId, syncRecurring, ...filters }, _api, _extra, baseQuery) => {
        if (syncRecurring) {
        ...
        return { data: await parseApiTransactions(transactions, userId) };
      },
